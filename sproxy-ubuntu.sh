...
# 支持安装、更新、卸载
# 支持 v1 和 v3 版本选择
# 适用于 systemd 系统（如 Debian、Ubuntu 等）
...
# systemd 服务文件路径
SERVICE_FILE="/etc/systemd/system/zbproxy.service"
...
# 检查依赖
check_dependencies() {
    local deps=("wget" "systemctl" "curl")
    local missing=()
...
download_and_install() {
    ...
    # 下载
    if ! wget -q -O /usr/local/bin/zbproxy "$url"; then
...
test_service() {
    ...
    # 先停止可能存在的服务
    systemctl stop zbproxy 2>/dev/null || true
    ...
    # 尝试启动服务
    if systemctl start zbproxy 2>/dev/null; then
        # 等待几秒让服务完全启动
        sleep 2
        
        # 检查服务状态
        if systemctl is-active --quiet zbproxy 2>/dev/null; then
...
install_zbproxy() {
    ...
    if [ -f "/usr/local/bin/zbproxy" ]; then
        ...
        # 停止旧服务
        systemctl stop zbproxy 2>/dev/null || true
        systemctl disable zbproxy 2>/dev/null || true
    fi
    ...
    if [ "$install_success" = true ]; then
        ...
        # 启用并启动服务
        echo "正在启用开机自启..."
        systemctl enable zbproxy 2>/dev/null || true
        
        # 启动服务（如果还没启动）
        if ! systemctl is-active --quiet zbproxy 2>/dev/null; then
            echo "正在启动服务..."
            systemctl start zbproxy 2>/dev/null || true
        fi
        
        # 检查服务状态
        echo "检查服务状态..."
        systemctl status zbproxy --no-pager 2>/dev/null || echo "服务状态检查完成"
        ...
        echo ""
        echo -e "常用命令:"
        echo -e "  启动: ${YELLOW}systemctl start zbproxy${NC}"
        echo -e "  停止: ${YELLOW}systemctl stop zbproxy${NC}"
        echo -e "  状态: ${YELLOW}systemctl status zbproxy${NC}"
        echo -e "  日志: ${YELLOW}journalctl -u zbproxy -f${NC}"
        return 0
    else
...
update_zbproxy() {
    ...
    if ! wget -q -O /usr/local/bin/zbproxy "$target_url"; then
...
uninstall_zbproxy() {
    ...
    # 检查是否已安装
    if [ ! -f "/usr/local/bin/zbproxy" ] && [ ! -f "$SERVICE_FILE" ]; then
    ...
    # 禁用开机自启
    echo "正在禁用开机自启..."
    if systemctl is-enabled --quiet zbproxy 2>/dev/null; then
        systemctl disable zbproxy 2>/dev/null || true
        echo "已从开机自启中移除"
    else
        echo "未找到开机自启配置，跳过"
    ...
    for file in "/usr/local/bin/zbproxy" "$SERVICE_FILE" "/run/zbproxy.pid" "$VERSION_FILE" "$VERSION_TYPE_FILE"; do
        if [ -f "$file" ]; then
...
show_status() {
    ...
    if command -v systemctl &>/dev/null; then
        echo "服务状态:"
        systemctl status zbproxy --no-pager 2>/dev/null || echo -e "${YELLOW}服务未注册${NC}"
    fi
